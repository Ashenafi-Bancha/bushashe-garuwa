    ...
    <main className="pt-20">
      ...
      <section className="relative h-[60vh] min-h-[400px] flex items-end overflow-hidden">
        ...
        <div className="relative z-10 max-w-screen-xl mx-auto px-4 sm:px-6 pb-16 w-full">
          <div className="text-[#C99A45] text-xs font-sans font-semibold tracking-[0.3em] uppercase mb-4">{hg.hero.eyebrow}</div>
          <h1 className="font-serif text-3xl sm:text-5xl lg:text-6xl font-light text-white leading-tight">{hg.hero.title}</h1>
        </div>
      </section>
            ...
            {categories.map((cat) => {
              ...
              <Link key={cat.id} to={cat.to} className="group bg-white overflow-hidden heritage-card">
                ...
                <div className="p-5">
                  ...
                  <h3 className="font-serif text-lg font-medium text-[#173F35] mb-2">{text.label}</h3>
      ...
      <section className="bg-[#173F35] py-12 sm:py-16 lg:py-24">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6">
          <div className="mb-8 sm:mb-12">
            <div className="text-[#C99A45] text-xs font-sans font-semibold tracking-[0.3em] uppercase mb-4">{hg.trees.eyebrow}</div>
            <h2 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-light text-white leading-tight">{hg.trees.title}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {trees.map((tree, i) => {
              ...
              <button
                ...
                className="text-left border border-white/10 hover:border-[#C99A45]/40 p-6 transition-all duration-300"
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="text-[#C99A45] font-serif text-2xl font-light mb-1">{tree.wolaytta}</div>
          ...
          <div className="mt-10 text-center">
            <div className="inline-flex items-center gap-3 border border-[#C99A45]/30 text-[#C99A45] text-sm font-sans px-6 py-3">
      ...
      <section className="bg-[#F6F1E7] py-12 sm:py-16 lg:py-24">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
            <div>
              <div className="text-[#A65A3A] text-xs font-sans font-semibold tracking-[0.3em] uppercase mb-4">{hg.family.eyebrow}</div>
              <h2 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-light text-[#173F35] leading-tight mb-6">{hg.family.title}</h2>
              <p className="text-[#1D211E]/70 font-sans text-base leading-relaxed mb-8">
                {hg.family.desc}
              </p>
              <Link to="/heritage/family" className="inline-flex items-center gap-2 bg-[#173F35] hover:bg-[#1e5447] text-white text-sm font-sans font-semibold tracking-widest uppercase px-8 py-4 transition-colors">
                {hg.family.cta}
              </Link>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {hg.family.generations.map((gen, i) => (
                <div key={i} className={`aspect-square bg-[#173F35]/10 flex flex-col items-center justify-center p-3 ${i === 2 ? 'col-span-1 row-span-2' : ''}`}>
